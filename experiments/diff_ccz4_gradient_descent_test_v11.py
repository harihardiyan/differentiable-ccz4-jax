‎
‎# FILE: diff_ccz4_gradient_descent_test_v11.py
‎# TEST 9: FINE-TUNING KE KAPPA1=2.0 (100 ITERASI)
‎
‎import jax
...
‎class AdamState:
‎    def __init__(self, count, m, v):
‎        self.count = count
‎        self.m = m
‎        self.v = v
‎    def replace(self, **kwargs):
‎        return AdamState(kwargs.get('count', self.count), kwargs.get('m', self.m), kwargs.get('v', self.v))
...
‎    return AdamState(jnp.array(0), tree_map(jnp.zeros_like, params), tree_map(jnp.zeros_like, params))
‎
‎def adam_update(i, grad, state, learning_rate=1e-3, b1=0.9, b2=0.999, eps=1e-8):
‎    state = state.replace(count=state.count + 1)
‎    i = state.count
‎    m = tree_map(lambda m_i, g: (b1 * m_i) + (1 - b1) * g, state.m, grad)
‎    v = tree_map(lambda v_i, g: (b2 * v_i) + (1 - b2) * (g * g), state.v, grad)
‎    m_hat = tree_map(lambda m_i: m_i / (1 - b1**i), m)
‎    v_hat = tree_map(lambda v_i: v_i / (1 - b2**i), v)
‎    update_tree = tree_map(lambda m_h, v_h: learning_rate * m_h / (jnp.sqrt(v_h) + eps), m_hat, v_hat)
‎    new_state = AdamState(state.count, m, v)
‎    return new_state, update_tree.kappa1 
‎
‎# =============================================================================
...
‎
‎# KAPPA1 AWAL: 1.9171
‎PARAMS = Params(kappa1=1.9171, kappa2=1.0, eta=2.0)
‎
‎TEST_TIMESTEPS = 200      
...
‎    """Loss: meminimalkan pelanggaran Constraint dan K."""
...
‎    # Loss: Constraint Evolution (Theta^2) dan K^2
‎    return jnp.mean(state.Theta**2) + 1e-8 * jnp.mean(state.K**2)
‎
‎# Gradien terhadap state awal (0) dan parameter (1)
...
‎    
‎    print(f"--- 🏆 DIFF-CCZ4 GRADIENT DESCENT TEST N={N_start}^3 / T={TEST_TIMESTEPS} (K1_INIT: {PARAMS.kappa1:.4f}, LR: {LEARNING_RATE:.0e}, PERTURBATION: 1e-2, ITER: {N_ITERATIONS}) ---")
‎    
‎    # --- Kompilasi Awal ---
...
‎        (grad_state_tuple, grad_params) = grad_loss(current_state_tuple, current_params, TEST_TIMESTEPS, GLOBAL_DT, dx, CHECKPOINT_INTERVAL)
‎        return loss_val, grad_params.kappa1, grad_params
‎
‎    # MAIN OPTIMIZATION LOOP
‎    for step in range(1, N_ITERATIONS + 1):
‎        start_step = time.time()
‎        
‎        # 1. Run JITted function untuk mendapatkan Loss dan Gradien
‎        loss_val, grad_kappa1, grad_params_tree = optimization_step(params_opt, state_tuple)
‎        
‎        # 2. Update Adam state dan dapatkan perubahan kappa1
‎        grad_for_adam = grad_params_tree 
‎
‎        opt_state, delta_kappa1 = adam_update(step, grad_for_adam, opt_state, learning_rate=LEARNING_RATE)
‎        
‎        # 3. Terapkan pembaruan pada parameter
‎        new_kappa1 = params_opt.kappa1 - delta_kappa1
‎        params_opt = params_opt.replace(kappa1=new_kappa1)
‎        
...
‎            print(f"Step {step:03d} | Loss: {loss_val:.2e} | Grad K1: {grad_kappa1:.2e} | Time: {step_time:.2f}s | K1: {params_opt.kappa1:.4f}")
‎        
‎    print("\n--- ✅ HASIL AKHIR OPTIMASI ---")
‎    print(f"Loss Awal (Step 1): {loss_history[0]:.2e}")
‎    print(f"Loss Akhir (Step {N_ITERATIONS}): {loss_history[-1]:.2e}")
‎    print(f"Nilai Akhir Kappa1: {params_opt.kappa1:.4f}")
‎    
‎    if params_opt.kappa1 > 1.95 and loss_history[-1] < loss_history[0] * 0.1:
‎        print("STATUS: SUCCESS FANTASTIC! Loss turun drastis dan K1 konvergen ke nilai fisik 2.0.")
‎    elif loss_history[-1] < loss_history[0]:
‎        print("STATUS: SUCCESS. Loss turun dan K1 bergerak ke arah yang benar.")
‎    else:
‎        print("STATUS: CAUTION. Periksa apakah Loss atau K1 bergerak ke arah yang salah.")
‎
